    ...
    try {
      ...
      for (final doc in docs) {
        ...
        final String originRoostId =
            (data['origin_roost_id'] ?? '').toString().trim();
      ...
      setState(() {
        loadedMessage =
            (selectedData['message'] ?? 'Message was empty.').toString();

        loadedHead = (selectedData['head'] ?? '').toString();
        loadedBody = (selectedData['body'] ?? '').toString();
        loadedLegs = (selectedData['legs'] ?? '').toString();
        loadedHealth = (selectedData['health'] ?? '').toString();
        loadedHops = (selectedData['hops'] ?? '').toString();
        loadedOriginRoostId = (selectedData['origin_roost_id'] ?? '').toString();
      });
